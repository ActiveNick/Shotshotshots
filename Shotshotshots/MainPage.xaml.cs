...
namespace Shotshotshots
{
    ...
    {
        // Speech, text & UI constants
        private const string DictationButtonLabelStart = " Start!";
        private const string DictationButtonLabelStop = " Stop";
        ...
        protected async override void OnNavigatedFrom(NavigationEventArgs e)
        {
            if (this.speechRecognizer != null)
            {
                if (isListening)
                {
                    ...
                    isListening = false;
                    DictationButtonText.Text = DictationButtonLabelStart;
        ...
        private async void ContinuousRecognitionSession_ResultGenerated(SpeechContinuousRecognitionSession sender, SpeechContinuousRecognitionResultGeneratedEventArgs args)
        {
            ...
            // For the drinking game, audio streams are not optimized for dictation so we'll even accept Low confidence
                ...
                args.Result.Confidence == SpeechRecognitionConfidence.High)
            {
                ...
                AnalyzeDictationResults(args.Result.Text);
        ...
        /// <summary>
        /// Anaslyze speech recognition results from dictation to detect special keywords
        /// </summary>
        /// <param name="recoResult">The text string containing the speech recognition result.</param>    
        private async void AnalyzeDictationResults(string recoResult)
        {           
            await dispatcher.RunAsync(CoreDispatcherPriority.Normal, () =>
            {
                string keyword = txtKeywords.Text.ToLower();
                recoResult = recoResult.ToLower();

                if (recoResult.Contains(keyword))
                {
                    keywordsTextBox.Text += ("; " + keyword);
                    SendShotTrigger();
                }
            });
        }
        ...
        {
            await SendShotTrigger();

        }

        private static async Task SendShotTrigger()
        {
            try
            {
                // Asynchronously call the custom API using the POST method. 
                var result = await App.MobileService.InvokeApiAsync("Shotshotshots", System.Net.Http.HttpMethod.Get, null);
            }
            catch (Exception exc)
            {
                var dlg = new Windows.UI.Popups.MessageDialog("Something went wrong with the Azure Mobile API call:"
